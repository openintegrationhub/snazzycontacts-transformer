...
module.exports.getExpression = (msg) => {
  console.log(`STREET: ${JSON.stringify(msg.body.addresses)}`);
  console.log(`HERE: ${msg.body.addresses[0].street}`);
  const expression = {
    ...
    "private_street":  "MailingStreet": jsonata(`$lookup($filter(msg.body.addresses, ${function($v) { $v.description = "mailing"}}), "street")`).evaluate(),
