...
module.exports.getExpression = (msg) => {
  ...
  let addresses = msg.body.addresses;

  function getAddresses(v, z) {
    v.description = z;
    return v;
  }
  ...
  const expression = {
    ...
    // "private_street": msg.body.addresses[0].street,
    // "private_street": jsonata(`$lookup($filter(${msg.body.addresses}, function(v) { v.description = "other"}), "street")), "street")`).evaluate(),
    "private_street": jsonata(`$filter(${JSON.stringify(msg.body.addresses)}, function($v) { $v.description = 'primary'})`).evaluate()[0].street,
    "private_zip_code": msg.body.addresses[0].zipCode,
    "private_town": msg.body.addresses[0].city,
    "import_key": "",
    ...
    "private_country": msg.body.addresses[0].country,
