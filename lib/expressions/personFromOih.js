...
module.exports.getExpression = (msg) => {
  ...
  const expression = {
    ...
    "phone": (jsonata(`$filter(${JSON.stringify(msg.body.contactData)}, function($v) { $v.description = 'primary'})`).evaluate()),
    "email": (jsonata(`$filter(${JSON.stringify(msg.body.contactData)}, function($v) { $v.description = 'private'})`).evaluate()),
    "fax": "",
    "mobile_phone": "",
    "remarks": "",
    "updated_by_user": "",
